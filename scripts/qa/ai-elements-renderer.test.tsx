...
describe("AI Elements renderer adapters", () => {
  ...
  it("covers the committed Markdown kitchen sink without weakening the renderer boundary", () => {
    ...
    for (const host of [restored.host, streaming.host]) {
      ...
      expect(host.querySelectorAll('[data-slot="code-block"]')).toHaveLength(3);
      expect(host.querySelector('[data-slot="code-block"][data-language="unknown"]')).not.toBeNull();
      expect(host.querySelectorAll('[data-slot="code-block"][data-language="text"]')).toHaveLength(2);
      expect(host.querySelectorAll('[aria-label="Copy code"]').length).toBeGreaterThanOrEqual(3);
