...
export function createTeamDatabaseSessionManager({ database, profile, teamChatId, agentId, session }) {
  const entries = session?.sessionId ? database.getTeamSessionEntries(teamChatId, agentId) : [];
  const header = entries[0];
  if (!header || header.type !== "session") throw new Error("Team session is missing its SQLite header.");
  const manager = SessionManager.inMemory(profile.workspace, { id: header.id });
  manager.sessionId = header.id;
  manager.sessionFile = session.sessionPath;
  manager.sessionDir = "";
  manager.fileEntries = entries;
  manager.flushed = true;
  manager._buildIndex();
  manager._persist = (entry) => {
    database.appendTeamSessionEntry(teamChatId, agentId, entry);
    manager.flushed = true;
  };
  return manager;
}

export function createTeamDatabaseSession({ database, profile, teamChatId, agentId }) {
  const manager = SessionManager.inMemory(profile.workspace);
  const sessionId = manager.getSessionId();
  const header = manager.getHeader();
  const session = database.createTeamSession({ teamChatId, agentId, sessionId, workspace: profile.workspace, entries: [header] });
  return createTeamDatabaseSessionManager({ database, profile, teamChatId, agentId, session });
}
