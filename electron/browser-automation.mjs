...
function pageScript(action, selector, text, expectedSignature, pageTarget) {
...
export function redactBrowserToolMessage(message) {
  ...
  const content = message.content.map((part) => {
    if (part?.type !== "toolCall" || part.name !== "browser" || !part.arguments || typeof part.arguments !== "object" || !Object.hasOwn(part.arguments, "text")) return part;
    changed = true;
    return { ...part, arguments: { ...part.arguments, text: "[redacted]" } };
  });
  ...
  return {
    ...
    async execute(params, signal, scope) {
      ...
      try {
        ...
        if (target && (target.tabKey !== operation.tab.key || target.pageGeneration !== tab.pageGeneration)) throw new BrowserAutomationError("stale-target", "The browser target is no longer available.");
        ...
        const work = Promise.resolve().then(() => {
          ...
          return action.action === "navigate"
            ? Promise.resolve(tab.contents.loadURL(action.url)).then(() => ({ url: action.url }))
            : tab.contents.executeJavaScript(pageScript(action.action, selector, action.text, expectedSignature, pageTarget), true);
        });
  ...
  return {
    ...
    async execute(_toolCallId, params, signal) {
      ...
      try {
        ...
        return { content: [{ type: "text", text: message }], details: { action, tabId: params?.tabId, code: error instanceof BrowserAutomationError ? error.code : "action-failed" }, isError: true };
      }
    },
  };
}
